...
import com.example.aquasegv2.ImageUtils.clone
import com.example.aquasegv2.ImageUtils.scaleMask
import com.example.aquasegv2.MetaData.extractNamesFromLabelFile
...
class InstanceSegmentation(
    ...
    fun emptyMask(): Array<FloatArray> {
        return Array(yPoints) { FloatArray(xPoints) { 0f } }
    }
    ...
    fun invoke(frame: Bitmap,withMask: Boolean=true) {
        ...
        val segmentationResults = if (withMask) {
            val maskProto = reshapeMaskOutput(maskProtoBuffer.floatArray)
            bestBoxes.map {
                SegmentationResult(
                    box = it,
    ...
    interface InstanceSegmentationListener {
        ...
        abstract fun segment(crop: Bitmap): Bitmap
    }
